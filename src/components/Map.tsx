...
export default function Map({
    ...
    // Reset the baseline on race change so switching races doesn't flash everything.
    ...
    const coveredKeys = useMemo(() => {
        ...
        return s;
    }, [precinctResults, shiftByWard, coalitionByWard]);
    ...
    const style = useCallback((feature: any, currentSelectedWard: { name: string; num: string } | null = null) => {
        ...
        if (overlayMode === 'COALITION') {
            ...
            const share = coalitionByWard?.[tk];
            if (share === undefined) {
                baseStyle = { fillColor: '#0f172a', weight: 0.5, opacity: 0.2, color: '#1e293b', fillOpacity: 0.08 };
            } else {
                let h: number, sat: number, l: number;
                if (share < 50) {
                    h = 0; const intensity = Math.min((50 - share) / 40, 1);
                    l = 92 - intensity * 44; sat = 10 + intensity * 80;
                } else {
                    h = 140; const intensity = Math.min((share - 50) / 40, 1);
                    l = 92 - intensity * 52; sat = 10 + intensity * 90;
                }
                baseStyle = { fillColor: `hsl(${h}, ${sat}%, ${l}%)`, weight: 1, opacity: 1, color: '#334155', fillOpacity: 0.82 };
            }
        ...
        return baseStyle;
    }, [resultsMap, candidateColors, overlayMode, focusedCandidate, projectionData, dormantPoolData, dropoffData, maxDormantPool, maxDropoff, simulateHighlightedWards, turnoutByWard, comparisonTurnoutByWard, shiftByWard, coalitionByWard]);
    ...
    }, [resultsMap, candidateColors, overlayMode, dormantPoolData, dropoffData, turnoutByWard, comparisonTurnoutByWard, shiftByWard, coalitionByWard]);
    ...
    const onEachFeature = useCallback((feature: any, layer: L.Layer) => {
        ...
        const isInteractive = hasData || hasSimData || hasShiftData || hasCoalitionData;
    ...
    }, [resultsMap, candidateColors, onWardHover, overlayMode, onWardClick, dormantPoolData, dropoffData, buildWardData, shiftByWard, coalitionByWard]);
        ...
        <>
            ...
            >
                ...
                {geoJsonData && (
                    <>
                        <GeoJSON
                            key={`${selectedWard ? selectedWard.num : 'all'}-${raceResult?.id || 'default'}-${overlayMode}-${Object.keys(candidateColors).length}-${historicalLabel || 'loading'}-${focusedCandidate || 'none'}-${projectionData ? Object.keys(projectionData).length : 0}-${dormantPoolData ? 'dp' : 'ndp'}-${dropoffData ? 'do' : 'ndo'}-${simulateHighlightedWards ? simulateHighlightedWards.size : 0}-${turnoutByWard ? Object.keys(turnoutByWard).length : 0}-${comparisonTurnoutByWard ? Object.keys(comparisonTurnoutByWard).length : 0}-${shiftByWard ? Object.keys(shiftByWard).length : 0}-${coalitionByWard ? Object.keys(coalitionByWard).length : 0}`}
                            data={geoJsonData}
                ...
                <div style={getSafeTooltipStyle(displayWard.x, displayWard.y)}>
                    <div style={{
                        ...
                        {displayWard.coalitionShare !== undefined && (
                            <div style={{ padding: '10px 14px 12px' }}>
                                <div style={{ fontSize: '11px', color: '#94a3b8', marginBottom: '6px' }}>Combined slate support</div>
                                <div style={{ fontSize: '26px', fontWeight: 800, color: displayWard.coalitionShare >= 50 ? '#4ade80' : '#f87171' }}>
                                    {displayWard.coalitionShare.toFixed(1)}%
                                </div>
                                <div style={{ height: '6px', background: '#1e293b', borderRadius: '3px', overflow: 'hidden', marginTop: '6px' }}>
                                    <div style={{ height: '100%', width: `${Math.min(displayWard.coalitionShare, 100)}%`, background: displayWard.coalitionShare >= 50 ? '#22c55e' : '#ef4444', borderRadius: '3px' }} />
                                </div>
